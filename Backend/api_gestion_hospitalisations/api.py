...
class HospitalizationCreateView(APIView):
    permission_classes = [IsDoctor]  # Exige que l'utilisateur soit un médecin pour accéder à cette vue
...
class HospitalizationDetailView(APIView):
    permission_classes = [IsDoctor, IsAdmin]  # Exige que l'utilisateur soit un médecin ou un administrateur pour accéder à cette vue
