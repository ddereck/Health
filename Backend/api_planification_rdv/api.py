...
from .serializers import AvailabilitySerializer, DoctorSerializer, AppointmentSerializer
...
class DoctorAvailabilityView(APIView):
    def get(self, request):
        ...
        availabilities = Doctor.objects.all()  # Exemple de récupération des disponibilités des médecins depuis la base de données
...
class DoctorSelectionView(APIView):
    def get(self, request):
        ...
        doctors = Doctor.objects.filter(specialty='Cardiologie')  # Exemple de récupération des médecins avec une spécialité spécifique
        serialized_doctors = DoctorSerializer(doctors, many=True)  # Exemple de sérialisation des médecins
